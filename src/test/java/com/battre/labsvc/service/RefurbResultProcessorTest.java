...
public class RefurbResultProcessorTest {
  ...
  @Mock private RefurbRecordsRepository refurbRecordsRepo;
  @Mock private BlockingQueue<RefurbResultRecord> resultQueue;
  @Mock private GrpcMethodInvoker grpcMethodInvoker;
  @InjectMocks private RefurbResultProcessor refurbResultProcessor;
  private AutoCloseable closeable;

  @BeforeEach
  public void setUp() {
    closeable = MockitoAnnotations.openMocks(this);
    refurbResultProcessor =
        new RefurbResultProcessor(
            labPlansRepo,
            refurbPlanRepo,
            refurbStationsRepo,
            refurbRecordsRepo,
  ...
  public void testProcessRefurbResults_Pass() throws Exception {
    RefurbResultRecord resultRecord =
    ...
    RefurbRecordType refurbRecord =
        new RefurbRecordType(
            1, // refurbPlanId
            2, // refurbStnId
    ...
    verify(labPlansRepo, times(1))
        .setPlanStatusesForPlanId(eq(2), eq(LabPlanStatusEnum.PASS.toString()));
  ...
  public void testProcessRefurbResults_FailRetry() throws Exception {
    ...
    verify(labPlansRepo, times(1))
        .setPlanStatusesForPlanId(
            eq(2), eq(LabPlanStatusEnum.REFURB_BACKLOG_RETRY.getStatusDescription()));
    verify(refurbStationsRepo, times(1)).markRefurbStnFree(eq(2), any(Timestamp.class));
  ...
  public void testProcessRefurbResults_FailReject() throws Exception {
    ...
    UpdateBatteryStatusResponse opsSvcResponse = UpdateBatteryStatusResponse.newBuilder().build();
