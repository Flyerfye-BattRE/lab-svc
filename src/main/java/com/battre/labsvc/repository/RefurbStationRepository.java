...
public interface RefurbStationRepository extends JpaRepository<RefurbStationType, Integer> {
  ...
  @Query(
      "SELECT rst.refurbStnId, rsct.stationClass "
  ...
  @Query("SELECT rst " + "FROM RefurbStationType AS rst " + "ORDER BY refurbStnId ")
  List<RefurbStationType> getRefurbStationLogs();

  @Transactional
  @Modifying
  @Query(
      "UPDATE RefurbStationType "
          + "SET inUse = true, activeBatteryId = :batteryId, lastActiveDate = :lastActiveDate "
          + "WHERE refurbStnId = :refurbStnId")
  void markRefurbStnInUse(
      @Param("refurbStnId") int refurbStnId,
  ...
  @Query(
      "UPDATE RefurbStationType "
          ...
          + "WHERE refurbStnId = :refurbStnId")
  void markRefurbStnFree(
      @Param("refurbStnId") int refurbStnId, @Param("lastActiveDate") Timestamp lastActiveDate);
}
